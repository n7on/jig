...
_entra_get_all() {
    ...
    while [[ -n "$url" ]]; do
        page=$(az rest --method GET --url "$url" 2>/dev/null) || {
            _message_error "Failed to fetch $url"
            return 1
        }
        result=$(echo "$page" | _exec_python entra paginate.py "$all")
...
entra_license() {
    ...
    local result
    result=$(_exec _entra_get_all "https://graph.microsoft.com/v1.0/subscribedSkus") || return 1
...
entra_license_plan_list() {
    ...
    echo "$result" \
        | _exec_python entra license_plans.py \
        | _output_render
}
